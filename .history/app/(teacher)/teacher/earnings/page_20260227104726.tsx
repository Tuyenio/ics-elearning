...
export default function TeacherEarningsPage() {
  ...
  const handleExport = () => {
    ...
    // Nếu xuất từ chart, chỉ lấy các khoản liên quan đến khóa học và thời gian đã chọn
    // (Đã filter ở trên)
    const headers = ["ID", "Học viên", "Email", "Khóa học", "Số tiền", "Phương thức", "Trạng thái", "Ngày"]
    const rows = exportData.map((p) => [
      p.id,
    ...
    const exportDate = new Date().toLocaleDateString("vi-VN")
    const bannerLines = [["Báo cáo: Doanh thu giáo viên"], [`Ngày xuất: ${exportDate}`], [`Khóa học: ${exportCourse === "all" ? "Tất cả" : exportCourse}`], [`Từ: ${exportDateFrom || "-"} Đến: ${exportDateTo || "-"}`]]
    const aoa = [...bannerLines, headers, ...rows]
