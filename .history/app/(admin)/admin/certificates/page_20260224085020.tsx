    ...
    try {
      ...
      return (
        <div className="min-h-screen w-full">
          {/* ...existing code for header, stats, search/filter... */}
          <div className="w-full space-y-8">
            ...
            <div className="bg-card dark:bg-slate-900/60 border border-border dark:border-slate-800 rounded-2xl overflow-visible">
              {isLoading ? (
                  ...
                  {filteredCertificates.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                      {filteredCertificates.map((cert) => (
                        <div
                          key={cert.id}
                          className={`relative bg-white/90 dark:bg-slate-900/70 border border-border dark:border-slate-800 rounded-2xl p-5 shadow-sm hover:shadow-lg transition-shadow ${openMenu === cert.id ? "z-20" : "z-0"}`}
                        >
                          ...
                          <div className="mt-4">
                            {/* Card preview: image, title, description, etc. */}
                          ...
                          {selectedCertificate?.id === cert.id && (viewMode === "view" || viewMode === "reject") && (
                            <div className="absolute inset-0 z-50 flex items-center justify-center">
                              <div className="w-full h-full bg-black/30 dark:bg-black/40 absolute inset-0 rounded-2xl" />
                              <div className="relative w-[340px] max-w-[90vw] bg-card dark:bg-slate-900 border border-border dark:border-slate-800 rounded-2xl shadow-2xl p-4 animate-fadeIn z-60">
                                {viewMode === "view" && (
                                  <div>
                ...
                ...existing code for issued certificates table...
              )}
            </div>
            ...
            <div className="p-6">
                  {filteredCertificates.map((cert) => (
                    ...
                    >
                      ...
                      {/* Card preview content */}
                      <div className="mt-4">
                        {/* ...existing card preview content (image, title, description, etc.)... */}
                      </div>
                      {/* Card actions */}
                      {cert.status === "pending" && (
                        <div className="mt-4 grid grid-cols-2 gap-2">
                      ...
                      {/* Overlay detail/reject panel directly on top of card */}
                      {selectedCertificate?.id === cert.id && (viewMode === "view" || viewMode === "reject") && (
                        <div className="absolute inset-0 z-50 flex items-center justify-center">
                          <div className="w-full h-full bg-black/30 dark:bg-black/40 absolute inset-0 rounded-2xl" />
                          <div className="relative w-[340px] max-w-[90vw] bg-card dark:bg-slate-900 border border-border dark:border-slate-800 rounded-2xl shadow-2xl p-4 animate-fadeIn z-60">
                            {viewMode === "view" && (
                              <div>
                                {/* ...detail panel content (title, info, close button, etc.)... */}
                              </div>
                            )}
                            {viewMode === "reject" && (
                              <div>
                                {/* ...reject panel content (reason input, actions, close button, etc.)... */}
                              </div>
                            )}
                          </div>
                        </div>
