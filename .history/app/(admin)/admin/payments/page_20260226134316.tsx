      ...
      const mapped: Payment[] = Array.isArray(rawList)
        ? rawList.map((p: any) => {
            ...
            return {
              ...
              {/* Payment Detail Modal - Desktop style update */}
              {selectedPayment && (
                <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 p-4">
                  <div className="bg-card dark:bg-slate-900 border border-border dark:border-slate-800 rounded-2xl shadow-2xl w-full max-w-xl overflow-y-auto relative">
                    {/* Close button */}
                    <button
                    ...
                    {/* Header */}
                    <div className="bg-primary/90 dark:bg-accent/80 rounded-t-2xl px-8 py-6 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                      <div>
                        <h2 className="text-lg md:text-xl font-bold text-white">Chi tiết giao dịch</h2>
                        <span className="block text-white/80 text-sm mt-1">Số tiền thanh toán</span>
                        <span className="block text-3xl font-bold text-white mt-1">₫{formatPrice(selectedPayment.amount)}</span>
                      </div>
