...
>
  Xem chi tiết
        ...
        {/* Payments Table */}
        <div className="hidden lg:block bg-red-50 dark:bg-red-900/40 border border-red-200 dark:border-red-700 rounded-2xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-red-200 dark:border-red-700 bg-red-100 dark:bg-red-900/60">
                  <th className="text-left py-4 px-6 font-semibold text-red-700 dark:text-red-300">Người dùng</th>
                  <th className="text-left py-4 px-6 font-semibold text-red-700 dark:text-red-300">Khóa học</th>
                  <th className="text-left py-4 px-6 font-semibold text-red-700 dark:text-red-300">Giảng viên</th>
                  <th className="text-left py-4 px-6 font-semibold text-red-700 dark:text-red-300">Số tiền</th>
                  <th className="text-left py-4 px-6 font-semibold text-red-700 dark:text-red-300">Phương thức</th>
                  <th className="text-left py-4 px-6 font-semibold text-red-700 dark:text-red-300">Trạng thái</th>
                  <th className="text-left py-4 px-6 font-semibold text-red-700 dark:text-red-300">Ngày</th>
                  <th className="text-left py-4 px-6 font-semibold text-red-700 dark:text-red-300">Chi tiết</th>
                </tr>
              </thead>
              <tbody>
                {filteredPayments.map((payment) => (
                  <tr
                    key={payment.id}
                    className="border-b border-red-200 dark:border-red-700 hover:bg-red-100 dark:hover:bg-red-900/60 transition-smooth"
                  >
                    <td className="py-4 px-6">
                      <div>
                        <p className="text-red-700 dark:text-red-200 font-medium">{payment.user}</p>
                        <p className="text-red-400 dark:text-red-300 text-xs">{payment.userEmail}</p>
                      </div>
                    </td>
                    <td className="py-4 px-6 text-red-700 dark:text-red-200 max-w-[200px] truncate">
                      {payment.course}
                    </td>
                    <td className="py-4 px-6 text-red-400 dark:text-red-300">
                      {payment.teacher}
                    </td>
                    <td className="py-4 px-6 text-red-700 dark:text-red-200 font-medium">
                      ₫{formatPrice(payment.amount)}
                    </td>
                    <td className="py-4 px-6">
                      <span className="px-2 py-1 bg-red-200 dark:bg-red-800 rounded text-red-700 dark:text-red-200 text-xs font-medium">
                        {payment.method}
                      </span>
                    </td>
                    <td className="py-4 px-6">
                        ...
                        className={`px-3 py-1 rounded-full text-xs font-medium ${
                          payment.status === "success"
                            ...
                            : payment.status === "pending"
                              ...
                              : "bg-red-500/80 text-white dark:bg-red-700 dark:text-red-100"
                        }`}
                      >
                        {payment.status === "success"
                          ? "Thành công"
                    ...
                    <td className="py-4 px-6 text-red-400 dark:text-red-300">{formatDate(payment.date)}</td>
                    <td className="py-4 px-6">
                      <button
                        onClick={() => setSelectedPayment(payment)}
                        className="p-2 hover:bg-red-200 dark:hover:bg-red-800 rounded-lg transition-smooth"
                      >
                        <Eye size={18} className="text-red-700 dark:text-red-200" />
                      </button>
                    </td>
                  </tr>
                ))}
          ...
          {filteredPayments.length === 0 && (
            <div className="py-12 text-center">
              <CreditCard size={48} className="mx-auto mb-4 text-red-300 opacity-50" />
              <p className="text-red-400 dark:text-red-300">Không tìm thấy giao dịch nào</p>
            </div>
          )}
        </div>
      </div>
          ...
          return createPortal(
            <div className="fixed inset-0 bg-black/60 z-[9999] flex items-center justify-center p-4">
              <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-md w-full shadow-xl">
                ...
                <p><b>Người dùng:</b> {payment.user}</p>
                <p><b>Email:</b> {payment.userEmail}</p>
                <p><b>Khóa học:</b> {payment.course}</p>
                <p><b>Giảng viên:</b> {payment.teacher}</p>
                <p><b>Số tiền:</b> ₫{formatPrice(payment.amount)}</p>
                <p><b>Phương thức:</b> {payment.method}</p>
                <p><b>Trạng thái:</b> {payment.status === "success" ? "Thành công" : payment.status === "pending" ? "Chờ xử lý" : "Thất bại"}</p>
                <p><b>Ngày:</b> {formatDate(payment.date)}</p>
                <button
