              ...
              {selectedPayment && (
                ...
                >
                  <div
                    ...
                    className="bg-gradient-to-b from-[#0B1220] to-[#070B14] border border-white/10 rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.6)] max-w-lg w-full max-h-[90vh] overflow-y-auto relative z-[10000]"
                  >
                    {/* Close button */}
                    <button
                      onClick={() => setSelectedPayment(null)}
                      className="absolute top-4 right-4 p-2 hover:bg-secondary dark:hover:bg-slate-800 rounded-lg transition-smooth z-10"
                      aria-label="Đóng"
                    >
                    ...
                    {/* Header */}
                    <div className="px-8 pt-8 pb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                      <div>
                        <h2 className="text-lg md:text-xl font-bold text-white">Chi tiết giao dịch</h2>
                      </div>
                      <div className="flex flex-col items-end md:items-center gap-2">
                        <span className="text-white/80 text-xs">Mã giao dịch</span>
                        <span className="text-white font-bold text-base tracking-wide">{selectedPayment.id}</span>
                      </div>
                    </div>
                      ...
                      <span
                        className={
                          selectedPayment.status === "success"
                            ? "inline-flex items-center px-4 py-1.5 rounded-full text-xs font-semibold mt-2 bg-green-500/10 text-green-400 border border-green-500/20"
                            : selectedPayment.status === "pending"
                              ? "inline-flex items-center px-4 py-1.5 rounded-full text-xs font-semibold mt-2 bg-yellow-500/10 text-yellow-400 border border-yellow-500/20"
                              : "inline-flex items-center px-4 py-1.5 rounded-full text-xs font-semibold mt-2 bg-red-500/10 text-red-400 border border-red-500/20"
                        }
                      >
                        {selectedPayment.status === "success"
                          ? "Thành công"
                    ...
                    {/* Divider banking style */}
                    <div className="h-px bg-white/10 my-4 mx-8" />

                    {/* Amount Card */}
                    <div className="px-8 pb-2">
                      <div className="bg-[#0E2236] rounded-2xl p-6 text-center border border-cyan-400/20">
                        <p className="text-muted-foreground dark:text-slate-400 text-sm">Số tiền thanh toán</p>
                        <p className="text-4xl font-extrabold text-cyan-400 tracking-tight">₫{formatPrice(selectedPayment.amount)}</p>
                      </div>
                    </div>
                    ...
                    <div className="p-8 pt-2 space-y-6">
                      {/* Thông tin người mua */}
                      <div className="bg-secondary dark:bg-slate-800/60 rounded-xl p-4">
                      ...
                      <div className="bg-secondary dark:bg-slate-800/60 rounded-xl p-4">
                        ...
                        <p className="text-foreground dark:text-white text-sm font-medium break-all">{selectedPayment.transactionId}</p>
                      </div>
                    </div>
                  </div>
                </div>
...
>
  ...
  className="
    ...
    overflow-y-auto relative z-[10000]
  "
>
