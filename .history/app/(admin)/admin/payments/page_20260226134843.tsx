...
export default function AdminPaymentsPage() {
  ...
  const loadPayments = async () => {
    ...
    try {
      ...
      const mapped: Payment[] = Array.isArray(rawList)
        ? rawList.map((p: any) => {
            ...
            return {
              ...
              course: course.title || course.name || "Không rõ",
              courseId: course.id || "",
              teacher: teacher.name || "Không rõ",
              teacherEmail: teacher.email || "",
              amount: Number(p.amount) || 0,
              method: normalizeMethod(p.method),
              status: normalizeStatus(p.status),
              date: paidAt ? new Date(paidAt).toISOString() : "",
            }
          })
        : []
      ...
      setPayments(mapped)
      setStats({
        totalRevenue: (statsRes as any)?.totalRevenue ?? 0,
        pendingTransactions: (statsRes as any)?.pendingTransactions ?? 0,
        completedTransactions: (statsRes as any)?.completedTransactions ?? 0,
        failedTransactions: (statsRes as any)?.failedTransactions ?? 0,
        totalTransactions: (statsRes as any)?.totalTransactions ?? 0,
      })
    } catch (err) {
  ...
  const handleExport = () => {
    const exportData = payments.filter(p => {
              ...
              <div className="animate-slideUp" style={{ animationDelay: "0.35s" }}>
                <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm border border-white/20 dark:border-slate-800/50 rounded-2xl p-6 shadow-lg h-36 flex flex-col justify-between items-center">
                  <div className="flex flex-col items-center w-full">
                    ...
                    <p className="text-xl font-bold text-yellow-600 dark:text-yellow-400 mt-1">₫{formatNumber(stats.pendingTransactions)}</p>
                  </div>
              ...
              <div className="animate-slideUp" style={{ animationDelay: "0.45s" }}>
                <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm border border-white/20 dark:border-slate-800/50 rounded-2xl p-6 shadow-lg h-36 flex flex-col justify-between items-center">
                  <div className="flex flex-col items-center w-full">
                    ...
                    <p className="text-xl font-bold text-foreground dark:text-white mt-1">{stats.completedTransactions}</p>
                  </div>
              ...
              <div className="animate-slideUp" style={{ animationDelay: "0.55s" }}>
                <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm border border-white/20 dark:border-slate-800/50 rounded-2xl p-6 shadow-lg h-36 flex flex-col justify-between items-center">
                  <div className="flex flex-col items-center w-full">
                    ...
                    <p className="text-xl font-bold text-foreground dark:text-white mt-1">{stats.totalTransactions}</p>
                  </div>
...
>
      ...
      <div className="grid grid-cols-2 gap-3 text-sm pt-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span>{formatDate(payment.date, "dd/MM/yyyy")}</span>
        </div>
        <div className="flex items-center gap-2">
              ...
              <tbody>
                {filteredPayments.map((payment) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-6 text-muted-foreground dark:text-slate-400">{formatDate(payment.date, "dd/MM/yyyy")}</td>
                    <td className="py-4 px-6">
                      <button
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-foreground dark:text-white">{formatDate(payment.date, "dd/MM/yyyy")}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground dark:text-slate-400 text-sm">Mã giao dịch</p>
