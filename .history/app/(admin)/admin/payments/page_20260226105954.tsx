                  ...
                  className={`inline-block px-3 py-1 rounded-full text-xs font-medium mt-2 ${
                    ...
                    <div className="block sm:hidden space-y-4">
                      {filteredPayments.length === 0 ? (
                        ...
                        filteredPayments.map(payment => (
                          <div key={payment.id} className="bg-white dark:bg-slate-900 rounded-xl p-4 shadow border border-border dark:border-slate-800">
                            <div className="flex items-center justify-between gap-3">
                              <div>
                                <p className="font-semibold text-foreground dark:text-white leading-snug line-clamp-2">{payment.user}</p>
                                <p className="text-xs text-muted-foreground dark:text-slate-400 mt-1">{payment.userEmail}</p>
                              </div>
                              <span
                                className={`px-3 py-1 rounded-full text-xs font-medium ${
                                  payment.status === "success"
                            ...
                            <div className="text-xs text-muted-foreground mt-2">Khóa học: <span className="font-medium text-foreground dark:text-white">{payment.course}</span></div>
                            <div className="text-xs text-muted-foreground mt-1">Giảng viên: <span className="font-medium text-foreground dark:text-white">{payment.teacher}</span></div>
                            <div className="flex items-center justify-between mt-3">
                              <div className="text-sm font-bold text-primary dark:text-accent whitespace-nowrap leading-tight">₫{formatPrice(payment.amount)}</div>
                              <div className="flex items-center gap-2">
                                <span className="px-2 py-1 bg-secondary dark:bg-slate-800 rounded text-foreground dark:text-white text-xs font-medium">{payment.method}</span>
                                <button
                                  onClick={() => setSelectedPayment(payment)}
                                  className="p-2 rounded-lg bg-primary/10 text-primary dark:text-accent text-xs font-medium"
                                >
                                  Xem
                                </button>
                              </div>
                            </div>
                            <div className="text-xs text-muted-foreground mt-2">Ngày: {formatDate(payment.date)}</div>
                          </div>
                        ))
                      )}
                    </div>
