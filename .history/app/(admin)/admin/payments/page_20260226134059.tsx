  ...
  Xem chi tiết
      ...
      {selectedPayment && (
        <div className="fixed inset-0 bg-black/60 z-[9999] flex items-center justify-center p-4">
          <div className="bg-slate-900 rounded-2xl max-w-lg w-full shadow-xl overflow-y-auto">
            {/* Header + Amount */}
            <div className="flex items-center justify-between px-6 pt-6 pb-4 bg-slate-800 rounded-t-2xl">
            ...
            <div className="px-6 pb-6 bg-slate-800 rounded-b-2xl">
              <div className="flex flex-col items-end -mt-12 mb-6">
                <div className="bg-slate-800 rounded-xl px-6 py-3 text-right shadow-lg">
                  <div className="text-slate-400 text-sm">Số tiền thanh toán</div>
                  <div className="text-3xl font-bold text-cyan-400">₫{formatPrice(selectedPayment.amount)}</div>
                </div>
              </div>
              {/* User Info */}
              <div className="bg-slate-800 rounded-xl p-5 mb-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-semibold text-white">Thông tin người mua</span>
                </div>
                <div className="space-y-1 text-sm">
                  <p className="text-white font-medium">{selectedPayment.user}</p>
                  <p className="text-slate-400">{selectedPayment.userEmail}</p>
                  <p className="text-slate-400">{selectedPayment.userPhone}</p>
                </div>
              </div>
              {/* Course Info */}
              <div className="bg-slate-800 rounded-xl p-5 mb-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-semibold text-white">Thông tin khóa học</span>
                </div>
                <div className="space-y-1 text-sm">
                  <p className="text-white font-medium">{selectedPayment.course}</p>
                  <p className="text-slate-400">Mã khóa học: {selectedPayment.courseId}</p>
                  <p className="text-slate-400">Giảng viên: {selectedPayment.teacher}</p>
                </div>
              </div>
              {/* Transaction Info */}
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div className="bg-slate-800 rounded-xl p-5">
                  ...
                  <p className="text-white font-medium">{formatDate(selectedPayment.date)}</p>
                </div>
              </div>
              {/* Transaction Reference */}
              <div className="bg-slate-800 rounded-xl p-5 mb-2">
                <p className="text-slate-400 text-xs mb-1">Mã tham chiếu giao dịch</p>
                <p className="text-cyan-400 text-sm font-medium">{selectedPayment.transactionId}</p>
              </div>
            </div>
          </div>
        </div>
