          ...
          {filteredUsers.map((user) => (
            ...
            >
              ...
              <div className="flex justify-center gap-3 mt-4">
                <button
                  onClick={() => {
                    // Only anchor modal on mobile
                    if (window.innerWidth < 1024) {
                      const card = mobileCardRefs.current[user.id];
                      if (card) {
                        const rect = card.getBoundingClientRect();
                        setMobileModalPos({ x: rect.left + window.scrollX, y: rect.bottom + window.scrollY, width: rect.width });
                        setViewUser(user);
          ...
          >
            ...
            {mobileModalType === 'edit' && mobileModalPos && window.innerWidth < 1024 && (
              <div
                className="fixed left-0 top-0 w-full h-full bg-black/60 z-[9998]"
                style={{ pointerEvents: 'auto' }}
                onClick={() => { setIsEditUserOpen(false); setEditUser(null); setMobileModalType(null); setMobileModalPos(null); }}
              />
            )}
            <div
              className={
