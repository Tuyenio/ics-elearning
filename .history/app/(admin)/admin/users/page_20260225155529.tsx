...
export default function AdminUsersPage() {
  ...
  // Mobile modal state (anchor to card)
  const [mobileViewUserId, setMobileViewUserId] = useState<number | null>(null);
  const [mobileEditUserId, setMobileEditUserId] = useState<number | null>(null);

  // Modal type/position for mobile edit modal (for compatibility with modal code below)
  const [mobileModalType, setMobileModalType] = useState<'edit' | null>(null);
...
const formatDate = (dateString?: string) => {
          ...
          {filteredUsers.map((user) => (
            ...
            >
              ...
              <div className="flex justify-center gap-3 mt-4">
                <button
                  onClick={() => setMobileViewUserId(user.id)}
                ...
                <button
                  onClick={(e) => {
                    setMobileEditUserId(user.id);
                    setMobileModalType('edit');
                    // Get position for modal anchoring
              ...
              {/* Modal Xem neo card */}
              {mobileViewUserId === user.id && (
                <>
                  <div className="fixed inset-0 z-[99999]" onClick={() => setMobileViewUserId(null)} />
                  <div className="absolute inset-x-0 top-full mt-3 z-[99999]">
                    <div className="bg-card dark:bg-slate-900 border border-border rounded-2xl shadow-xl max-h-[80vh] overflow-y-auto p-4">
                      {/* Nội dung modal xem user (copy từ modal root) */}
                      <div className="flex flex-col items-center gap-4 mb-4">
                        ...
                        <div className="text-center">
                          <h3 className="text-2xl font-bold text-foreground dark:text-white">{user.name}</h3>
                          <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium mt-1 ${user.role === "teacher" ? "bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400" : user.role === "admin" ? "bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400" : "bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300"}`}>{user.role === "teacher" ? "Giảng viên" : user.role === "admin" ? "Quản trị viên" : "Học viên"}</span>
                          <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium mt-1 ml-2 ${user.status === "active" ? "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400" : "bg-gray-100 dark:bg-gray-900/30 text-gray-700 dark:text-gray-400"}`}>{user.status === "active" ? "Hoạt động" : user.status === "pending" ? "Chờ xác thực" : "Vô hiệu hóa"}</span>
                        </div>
                      </div>
                      <div className="space-y-2 text-sm mb-4">
                        <InfoRow label="Số điện thoại" value={user.phone || "Chưa cập nhật"} />
                        <InfoRow label="Email" value={user.email} />
                        <InfoRow label="Ngày tham gia" value={user.createdAt ? formatDate(user.createdAt) : "Chưa cập nhật"} />
                        <InfoRow label="Khóa học" value={user.courses || "Chưa cập nhật"} />
                        <InfoRow label="Chứng chỉ" value={user.certificates || 0} />
                        <InfoRow label="Tổng giờ học" value={user.totalHours ? `${user.totalHours}h` : "Chưa cập nhật"} />
                        {user.address && <InfoRow label="Địa chỉ" value={user.address} />}
                      ...
                      <div className="flex gap-3 pt-2 border-t border-border dark:border-slate-800">
                        <button
                          onClick={() => setMobileViewUserId(null)}
              ...
              {/* Modal Sửa neo card */}
              {mobileEditUserId === user.id && typeof window !== 'undefined' && createPortal(
                <>
                  {/* Backdrop overlay with blur and darken, max z-index */}
                  <div className="fixed inset-0 z-[2147483647] bg-black/60 backdrop-blur-sm" onClick={() => setMobileEditUserId(null)} />
                  <div className="fixed inset-x-0 top-0 mt-3 z-[2147483647] flex justify-center items-start pointer-events-none">
                    <div className="bg-card dark:bg-slate-900 border border-border rounded-2xl shadow-2xl max-h-[80vh] overflow-y-auto w-full mx-2 sm:mx-auto sm:max-w-lg relative animate-fadeInUp pointer-events-auto" style={{ boxShadow: '0 8px 40px 0 rgba(0,0,0,0.25)' }}>
                      <EditUserModal
                        user={user}
                        onClose={() => setMobileEditUserId(null)}
                        onSubmit={async (data) => {
  ...
  >
      ...
      {((window.innerWidth < 1024 && mobileModalType === 'edit' && mobileModalPos && isEditUserOpen && editUser) || (window.innerWidth >= 1024 && isEditUserOpen && editUser)) && (
        <>
          <div
            className={
