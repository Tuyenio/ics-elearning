...
export default function AdminUsersPage() {
  ...
  const [viewPos, setViewPos] = useState<{ top: number; left: number; width: number } | null>(null)
        ...
        <div className="lg:hidden space-y-4">
          {filteredUsers.map((user) => (
            ...
            >
              ...
              {/* MODAL XEM CHI TIẾT: viewUser + viewPos, anchored to card */}
              {viewUser && viewPos && (
                <>
                  {/* Overlay */}
                  <div
                  ...
                  />
                  {/* Modal – anchored to card */}
                  <div
                    className="fixed z-[9999]"
                    style={{
                      top: viewPos.top,
                      left: Math.max(12, viewPos.left),
                      width: Math.min(viewPos.width, typeof window !== 'undefined' ? window.innerWidth - 24 : viewPos.width),
                    }}
                  >
                    <div className="bg-card dark:bg-slate-900 border border-border dark:border-slate-800 rounded-2xl shadow-2xl max-h-[85vh] overflow-y-auto">
                      {/* HEADER */}
                      <div className="p-4 border-b border-border dark:border-slate-800 flex items-center justify-between">
                        <h2 className="text-lg font-bold text-foreground dark:text-white">Thông tin chi tiết</h2>
                        <button
                          onClick={() => {
                            setViewUser(null)
                            setViewPos(null)
                      ...
                      <div className="p-4 space-y-3 text-sm">
                        ...
                        <InfoRow label="Trạng thái" value={viewUser.status} highlight />
                        {viewUser.address && <InfoRow label="Địa chỉ" value={viewUser.address} />}
                        {viewUser.bio && <InfoRow label="Giới thiệu" value={viewUser.bio} />}
                      </div>
                      {/* FOOTER */}
                      <div className="p-4 border-t border-border dark:border-slate-800">
                        <button
                          ...
                          className="w-full py-2 rounded-lg bg-secondary text-sm font-medium"
                        >
                          Đóng
                        </button>
                      </div>
