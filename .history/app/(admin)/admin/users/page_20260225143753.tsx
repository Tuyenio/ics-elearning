...
function DropdownFilter({ options, value, onChange, className = "", width = 180 }: DropdownFilterProps) {
  ...
  return (
    <div ref={ref} className={`relative ${className}`} style={{ minWidth: width }}>
      ...
      <div
        className={`absolute left-0 mt-2 w-full z-[1050] rounded-xl shadow-lg bg-card dark:bg-[#181f2a] border border-border dark:border-slate-700 overflow-hidden transition-all duration-200 ${open ? "max-h-60 opacity-100 scale-100" : "max-h-0 opacity-0 scale-95 pointer-events-none"}`}
