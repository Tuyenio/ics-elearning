...
const formatDate = (dateString?: string) => {
  ...
  return (
    ...
    <div className="min-h-screen w-full">
                      ...
                      <button
                        onClick={e => {
                          ...
                          if (openMenu === user.id) {
                            ...
                            if (left < 8) left = 8;
                            setMenuPos({ x: left, y: rect.bottom });
                            setOpenMenu(user.id);
                          }
                        }}
      ...
      {openMenu && menuPos && (
        <div
          className="fixed z-[9999] bg-card dark:bg-slate-900 border border-border dark:border-slate-800 rounded-lg shadow-lg min-w-48"
          style={{ top: menuPos.y + 8, left: menuPos.x }}
          tabIndex={-1}
          onBlur={e => {
            // Only close if focus leaves the menu entirely
