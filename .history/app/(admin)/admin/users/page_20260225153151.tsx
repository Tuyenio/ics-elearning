...
const formatDate = (dateString?: string) => {
  ...
  function setMobileUser(user: UserData | null) {
  ...
  >
      ...
      {/* ================= MODALS/OVERLAYS AT ROOT (ALWAYS ON TOP) ================= */}
      {setMobileUser && mobileMode === 'view' && (
        <div className="fixed inset-0 z-[99999] bg-black/60 flex items-end">
          <div className="w-full max-h-[90vh] rounded-t-2xl bg-card p-4 overflow-y-auto">
            <button
            ...
            <div className="font-semibold text-lg mb-2">{mobileUser.name}</div>
            <div>Email: {setMobileUser.email}</div>
            <div>SĐT: {setMobileUser.phone}</div>
          </div>
        </div>
      )}
      {mobileUser && mobileMode === 'edit' && (
        <div className="fixed inset-0 z-[99999] bg-black/60 flex items-center justify-center p-2">
          <div className="w-full max-h-[95vh] bg-card rounded-2xl overflow-y-auto">
            <EditUserModal
              user={mobileUser}
              onClose={() => {
                setMobileUser(null);
