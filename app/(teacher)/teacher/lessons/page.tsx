...
export default function TeacherLessonsPage() {
  ...
  const handleDeleteLesson = async (id: string) => {
    ...
    } catch { toast.error("Xóa bài học thất bại") }
  }
  ...
  const handleTogglePublish = async (lesson: Lesson) => {
    try {
      ...
      const updated: Lesson = await res.json()
      setLessons((prev) => prev.map((l) => (l.id === updated.id ? updated : l)))
      toast.success(updated.isPublished ? "Đã công bố bài học" : "Đã ẩn bài học")
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground dark:text-white">Quản lý bài học</h1>
          <p className="text-muted-foreground dark:text-slate-400 mt-1">Kéo và thả để sắp xếp thứ tự bài học</p>
        </div>
        <button
      ...
      <div className="bg-card dark:bg-slate-900/60 border border-border dark:border-slate-800 rounded-xl p-4">
        <label className="block text-sm font-medium text-foreground dark:text-white mb-2">Chọn khóa học</label>
        {isLoadingCourses ? (
      ...
      {isLoadingLessons ? (
        ...
        <div className="text-center py-16 text-muted-foreground dark:text-slate-400">
          ...
          <button onClick={() => setShowAddModal(true)} className="mt-4 px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg text-sm font-medium">Thêm bài học đầu tiên</button>
        </div>
      ) : (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-0.5">
                  ...
                  {lesson.isFree && <span className="text-xs px-2 py-0.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-full">Miễn phí</span>}
                </div>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <button
                  ...
                  title={lesson.isPublished ? "Ẩn bài học" : "Công bố bài học"}
                >
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-card dark:bg-slate-900 border border-border dark:border-slate-700 rounded-2xl w-full max-w-lg">
            <div className="flex items-center justify-between p-6 border-b border-border dark:border-slate-700">
              <h3 className="text-lg font-semibold text-foreground dark:text-white">Thêm bài học mới</h3>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-foreground dark:text-white mb-1">Tên bài học *</label>
                <input type="text" value={newLesson.title} onChange={(e) => setNewLesson({ ...newLesson, title: e.target.value })} placeholder="Nhập tên bài học" className="w-full px-4 py-2 bg-background dark:bg-slate-950 border border-border dark:border-slate-800 rounded-lg text-foreground dark:text-white focus:outline-none focus:ring-2 focus:ring-primary" />
              </div>
              <div>
                <label className="block text-sm font-medium text-foreground dark:text-white mb-1">Mô tả</label>
                <textarea value={newLesson.description} onChange={(e) => setNewLesson({ ...newLesson, description: e.target.value })} placeholder="Mô tả ngắn về bài học" rows={3} className="w-full px-4 py-2 bg-background dark:bg-slate-950 border border-border dark:border-slate-800 rounded-lg text-foreground dark:text-white focus:outline-none focus:ring-2 focus:ring-primary" />
              </div>
              <div>
                <label className="block text-sm font-medium text-foreground dark:text-white mb-1">Loại bài học</label>
              ...
              <div className="flex items-center gap-2">
                ...
                <label htmlFor="isFree" className="text-sm text-foreground dark:text-white">Bài học miễn phí (xem trước)</label>
              </div>
            </div>
            <div className="flex items-center justify-end gap-3 p-6 border-t border-border dark:border-slate-700">
              <button onClick={() => setShowAddModal(false)} className="px-4 py-2 border border-border dark:border-slate-800 rounded-lg text-foreground dark:text-white hover:bg-secondary dark:hover:bg-slate-800">Hủy</button>
              <button onClick={handleAddLesson} disabled={isSaving} className="flex items-center gap-2 px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg font-medium disabled:opacity-60">
                {isSaving ? <Loader2 size={16} className="animate-spin" /> : <Plus size={16} />} Thêm bài học
              </button>
            </div>
          </div>
        </div>
      ...
      {editingLesson && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-card dark:bg-slate-900 border border-border dark:border-slate-700 rounded-2xl w-full max-w-lg">
            <div className="flex items-center justify-between p-6 border-b border-border dark:border-slate-700">
              <h3 className="text-lg font-semibold text-foreground dark:text-white">Chỉnh sửa bài học</h3>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <label htmlFor="editIsFree" className="text-sm text-foreground dark:text-white">Bài học miễn phí (xem trước)</label>
              </div>
            </div>
            <div className="flex items-center justify-end gap-3 p-6 border-t border-border dark:border-slate-700">
              <button onClick={() => setEditingLesson(null)} className="px-4 py-2 border border-border dark:border-slate-800 rounded-lg text-foreground dark:text-white hover:bg-secondary dark:hover:bg-slate-800">Hủy</button>
              <button onClick={handleUpdateLesson} disabled={isSaving} className="flex items-center gap-2 px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg font-medium disabled:opacity-60">
                {isSaving ? <Loader2 size={16} className="animate-spin" /> : <Check size={16} />} Lưu thay đổi
              </button>
            </div>
          </div>
        </div>
