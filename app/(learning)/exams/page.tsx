...
export default function StudentExamsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true)
      try {
        const list = await apiClient.getAvailableExams()
        setExams(Array.isArray(list) ? list : [])
      } catch (error) {
        const message = error instanceof Error ? error.message : "Không thể tải danh sách bài thi"
        toast.error(message)
      } finally {
        setLoading(false)
      }
  ...
  const timeNotice = (exam: ExamItem) => {
    ...
    return "Có thể vào thi ngay"
  }

  if (loading) {
    return <div className="p-6">Đang tải danh sách bài thi...</div>
  }
      ...
      <div className="relative overflow-hidden p-8 rounded-3xl animate-fadeIn" style={{ backgroundImage: "url('/image/exam2.png')", backgroundSize: "cover", backgroundPosition: "center" }}>
        ...
        <div className="relative z-10 space-y-8">
          <div className="animate-slideDown" style={{ animationDelay: "0.15s" }}>
            <h1 className="text-3xl font-bold text-white mb-2 drop-shadow-lg">Bài thi của tôi</h1>
            <p className="text-black/70 dark:text-white/80 drop-shadow">Quản lý các khóa học bạn đã đăng ký.</p>
          </div>
          ...
          <div key={exam.id} className="rounded-2xl border bg-card p-5">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">{exam.course?.title || "Khóa học"}</p>
                ...
                <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                  ...
                  <span className="inline-flex items-center gap-1"><Trophy size={14} /> {exam.type === "official" ? "Thi thật" : "Thi thử"}</span>
                </div>
