    ...
    <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white' : 'bg-gradient-to-br from-sky-50 via-blue-50 to-cyan-50 text-gray-900'}`}>
      <div className="flex h-screen">
        ...
        >
          <div className="p-8 space-y-8 max-w-7xl mx-auto">
            ...
            <div className="flex items-center justify-between">
              <div className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>ICS-Learning</div>
            </div>

            {/* Top Balance Section */}
            <div className="grid grid-cols-3 gap-4">
              <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-2xl p-6 border shadow-sm hover:shadow-md transition-all`}>
                <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} mb-2`}>TỔNG SỐ DƯ</p>
                <p className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{formatCurrency(totalSpent)}</p>
              </div>
              <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-2xl p-6 border shadow-sm hover:shadow-md transition-all flex items-center justify-between`}>
                <div>
              ...
              <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-2xl p-6 border shadow-sm hover:shadow-md transition-all`}>
                ...
                <p className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{formatCurrency(dailySpend)}</p>
              </div>
            </div>
            ...
            <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-2xl p-6 border shadow-sm`}>
              <div className="flex items-center justify-between mb-6">
                <h2 className={`text-xl font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Giao dịch gần đây</h2>
                <div className="relative w-64">
                  ...
                  <input
                    ...
                    className={`w-full pl-10 pr-4 py-2 ${isDarkMode ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-500' : 'bg-gray-50 border-gray-200 text-gray-900 placeholder-gray-400'} border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400`}
                  />
                </div>
              </div>
              ...
              {filteredPayments.length === 0 ? (
                ...
                <div className="space-y-2">
                  {/* Table Header */}
                  <div className={`grid grid-cols-12 gap-4 px-4 py-3 text-sm ${isDarkMode ? 'text-gray-400 border-gray-700' : 'text-gray-600 border-gray-200'} font-medium border-b`}>
                  ...
                  {filteredPayments.map((payment) => (
                    <motion.div
                      ...
                      className={`grid grid-cols-12 gap-4 px-4 py-4 ${isDarkMode ? 'bg-gray-700 border-gray-600 hover:bg-gray-600' : 'bg-gray-50 border-gray-100 hover:bg-blue-50'} rounded-lg transition-all border hover:border-blue-200`}
                    >
                      <div className={`col-span-4 text-sm line-clamp-1 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{payment.courseTitle}</div>
                      <div className={`col-span-2 text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>{formatDate(payment.enrolledAt)}</div>
                      ...
                      <div className={`col-span-2 font-semibold text-sm ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{formatCurrency(payment.finalAmount)}</div>
        ...
        <motion.div
          ...
          className={`w-64 ${isDarkMode ? 'bg-gray-800/70 border-gray-700' : 'bg-white/70 border-gray-200'} backdrop-blur-md border-l p-6 overflow-y-auto [&::-webkit-scrollbar]:hidden [&::-moz-scrollbar]:hidden [-ms-overflow-style:none] space-y-6`}
        >
          <h3 className={`text-lg font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Khóa học của bạn</h3>
          <div className="space-y-4">
            {payments.slice(0, 3).map((payment) => (
              <motion.div
                ...
                onClick={() => handleViewDetails(payment)}
                className={`${isDarkMode ? 'bg-gray-700 border-gray-600 hover:bg-gray-600' : 'bg-white border-gray-200 hover:shadow-md'} rounded-xl p-4 cursor-pointer transition-all border`}
              >
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-orange-400 to-orange-600"></div>
                  <div>
                    <h4 className={`text-sm font-semibold line-clamp-1 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{payment.courseTitle}</h4>
                ...
                <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} mt-2`}>75% hoàn thành</p>
              </motion.div>
            ))}
          </div>

          <button className="w-full py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg font-medium transition-all flex items-center justify-center gap-2">
            <span>+</span>
            Thêm khóa học
          </button>
        </motion.div>
      ...
      {viewingDetails && selectedPayment && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="sticky top-0 bg-gradient-to-r from-blue-500 to-cyan-500 border-b text-white p-6">
              <div className="flex items-center justify-between">
            ...
            <div className="p-6 space-y-6">
              <div>
                <h3 className={`text-xl font-semibold mb-2 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{selectedPayment.courseTitle}</h3>
              ...
              <div className="space-y-4">
                <div>
                  <h4 className={`font-semibold mb-3 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Thông tin thanh toán</h4>
                ...
                <div className={`border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} pt-4`}>
                  <h4 className={`font-semibold mb-3 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Chi tiết giá</h4>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                    ...
                    <div className={`flex justify-between text-lg font-bold border-t ${isDarkMode ? 'border-gray-700 text-white' : 'border-gray-200 text-gray-900'} pt-2`}>
              ...
              <div className="flex gap-3 pt-4">
                {selectedPayment.courseSlug && (
                  <Link
                    ...
                    className="flex-1 px-4 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg hover:shadow-lg transition-all text-center font-semibold"
                  >
                    Xem khóa học
                  </Link>
                )}
                {selectedPayment.status === "completed" && (
                  <button
                    ...
                    className={`flex-1 px-4 py-3 ${isDarkMode ? 'bg-gray-700 hover:bg-gray-600 text-white border-gray-600' : 'bg-gray-100 hover:bg-gray-200 text-gray-900 border-gray-200'} rounded-lg transition-all flex items-center justify-center gap-2 font-semibold border`}
                  >
                    <Download size={20} />
                    Tải hóa đơn
                  </button>
