  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-border bg-card p-5 space-y-4">
        <h2 className="text-xl font-semibold flex items-center gap-2"><Plus size={18} /> {t("adm_sub_create_plan", "Tạo gói mới")}</h2>
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1">{t("adm_sub_duration", "Thời hạn (tháng)")}</label>
          ...
          <div>
            <label className="block text-sm font-medium mb-1">{t("adm_sub_course_limit", "Giới hạn khóa học")}</label>
          ...
          <div>
            <label className="block text-sm font-medium mb-1">{t("adm_sub_storage", "Dung lượng lưu trữ (GB)")}</label>
          ...
          <div>
            <label className="block text-sm font-medium mb-1">{t("adm_sub_student_limit", "Giới hạn học viên")}</label>
        ...
        <div className="space-y-3">
          {plans.map((plan) => (
            <div key={plan.id} className="rounded-lg border border-border p-4 space-y-3">
              <div className="grid md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-medium text-muted-foreground mb-1">{t("adm_sub_plan_name", "Tên gói")}</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-muted-foreground mb-1">{t("adm_sub_price", "Giá (VND)")}</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-muted-foreground mb-1">{t("adm_sub_duration", "Thời hạn (tháng)")}</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-muted-foreground mb-1">{t("adm_sub_course_limit", "Giới hạn khóa học")}</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-muted-foreground mb-1">{t("adm_sub_storage_short", "Dung lượng (GB)")}</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-muted-foreground mb-1">{t("adm_sub_student_limit", "Giới hạn học viên")}</label>
              ...
              <div className="flex gap-2 pt-2 border-t border-border">
                <button className="px-3 py-1 rounded bg-emerald-600 text-white flex items-center justify-center gap-1" onClick={() => updatePlan(plan.id, plan)}><Save size={14} /> {t("adm_sub_save", "Lưu")}</button>
                <button className="px-3 py-1 rounded bg-red-500 text-white flex items-center justify-center gap-1" onClick={() => deletePlan(plan.id)}><Trash2 size={14} /> {t("adm_sub_delete_btn", "Xóa")}</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="rounded-2xl border border-border bg-card p-5 space-y-3">
        ...
        <ul className="list-disc pl-5 text-sm text-muted-foreground space-y-1">
          <li>{t("adm_sub_mod_courses", "Quản lý khóa học")}: /admin/courses</li>
          <li>{t("adm_sub_mod_categories", "Quản lý nội dung (categories)")}: /admin/categories</li>
          <li>{t("adm_sub_mod_reports", "Quản lý báo cáo")}: /admin/reports</li>
          <li>{t("adm_sub_mod_settings", "Cấu hình hệ thống")}: /admin/settings</li>
          <li>{t("adm_sub_mod_payments", "Thanh toán tổng hợp")}: /admin/payments</li>
        </ul>
      </section>
    </div>
  )
