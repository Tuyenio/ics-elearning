...
export function GoogleLoginButton({ className = '', size = 'md' }: GoogleLoginButtonProps) {
  ...
  return (
    <button
      ...
      disabled={loading}
      className={`w-full ${sizeClasses[size]} flex items-center justify-center gap-3 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm hover:shadow-md ${className}`}
    >
