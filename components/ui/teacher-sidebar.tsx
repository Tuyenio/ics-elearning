    ...
    <>
      ...
      >
        ...
        <div className="flex-shrink-0 px-4 py-5 border-b border-border/50 dark:border-slate-800/50 mt-12 md:mt-0">
          ...
          {!isCollapsed && (
            <div className="mt-2 text-center">
              <h3 className="text-sm font-bold text-foreground dark:text-white">{t("teacher_portal", "Teacher Portal")}</h3>
              ...
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-300 ease-out group relative ${
                  isActive
                    ...
                    : "text-muted-foreground dark:text-slate-400 hover:text-foreground dark:hover:text-white hover:bg-secondary/60 dark:hover:bg-slate-800/60"
                }`}
                title={isCollapsed ? item.label : ""}
              >
                {isActive && <span className="absolute left-0 top-2 bottom-2 w-1 rounded-full bg-primary dark:bg-accent" />}
        ...
        <div className="flex-shrink-0 px-3 py-4 border-t border-border/50 dark:border-slate-800/50 space-y-2.5 bg-card/50 dark:bg-slate-900/50 backdrop-blur-sm">
          ...
          {user && (isHovering || !isCollapsed) && (
            ...
            >
              <div className="flex items-center gap-3">
                <UserAvatar 
                  src={user.avatar}
                  name={user.name || t("role_teacher", "Giảng viên")}
                  size="md"
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-foreground dark:text-white font-semibold text-sm truncate group-hover:text-primary dark:group-hover:text-accent transition-colors">
                    {user.name || t("role_teacher", "Giảng viên")}
                  </p>
          ...
          {user && isCollapsed && !isHovering && (
            <Link
              href="/teacher/profile"
              className="flex justify-center"
              title={user.name || t("role_teacher", "Giảng viên")}
            >
              <UserAvatar 
                src={user.avatar}
                name={user.name || t("role_teacher", "Giảng viên")}
          ...
          <div className="relative">
            ...
            >
              ...
              {(isHovering || !isCollapsed) && <span className="font-medium text-sm">{t("nav_logout", "Đăng xuất")}</span>}
            </button>
