...
export function TeacherRouteGuard({ children }: TeacherRouteGuardProps) {
  ...
  const unauthorized = !loading && (!isAuthenticated || !isTeacher)

  useEffect(() => {
    if (!unauthorized) return

    const next = encodeURIComponent(pathname || "/teacher/dashboard")
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background dark:bg-slate-950">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground dark:text-slate-400">{t("common_loading", "Đang tải...")}</p>
        </div>
      </div>
    )
  }
      ...
      <div className="min-h-screen flex items-center justify-center bg-background dark:bg-slate-950 p-4">
        <div className="max-w-md w-full rounded-2xl border border-border dark:border-slate-800 bg-card dark:bg-slate-900/60 p-6 text-center">
          ...
          <p className="text-sm text-muted-foreground dark:text-slate-400">
            {t("teacher_auth_required", "Phiên đăng nhập không hợp lệ. Đang chuyển đến trang đăng nhập...")}
          </p>
        </div>
      </div>
    )
